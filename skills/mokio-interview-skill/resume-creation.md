...
- 背景与解决方案混在一起——分开写更清晰
- 学习型背景（"为了学习XX技术"）——用业务视角替代（见下方包装指南）

### 项目包装指南

**项目背景必须用业务视角，不能用学习视角。** 即使是个人/课程项目，也要从业务需求角度描述。

```
❌ 学习视角："为了学习 Go 语言，我开发了一个异步任务框架"
✅ 业务视角："业务需要支持任务自动调度与重试，设计了 AsyncFlow 框架"

❌ 学习视角："用 Spring Boot 做了一个短链系统练手"
✅ 业务视角："长链转短链需求下，实现高性能短链服务，支持多级缓存架构"
```

**包装公式：** `业务型背景 = 场景 + 痛点 + 为什么需要这个系统`

**如何推导业务痛点（不造假）：**
1. 从项目功能反推——项目做了什么？之前没有它会怎样？
2. 从技术难点反推——解决了什么技术问题？那个问题在真实场景中会造成什么后果？
3. 从性能优化反推——做了压测/优化？说明有性能需求
4. 从架构决策反推——选了A不选B？说明有选型理由

**个人职责必须用工程型描述，不能用技术栈罗列。**

```
❌ "使用了 MySQL 存储任务数据，Redis 做缓存"
✅ "数据库表设计：仅涉及低耦合的三张数据库表，方便任务快速注册、检索和管理"

❌ "用 Redis 分布式锁解决并发问题"
✅ "多机竞争优化：多 worker 拉取任务容易拉到同一批，MySQL 行级锁冲突时 CPU 飙高，
   最终引入 Redis 分布式锁解决，下阶段考虑引入 MQ 实现解耦"
```

**工程型描述 = 技术选型 + 问题 + 方案演进 + 结论/下一步**

**技术难点是面试追问的核心入口，每个项目必须包含。**
...
- [ ] 专业技能部分列出项目中实际发现的技术
- [ ] 项目背景全部为业务视角（无"为了学习"、"练手"等学习型背景）
- [ ] 每个项目的个人职责包含技术选型理由（不是纯技术栈罗列）
- [ ] 每个项目包含技术难点部分（面试追问入口）
- [ ] 没有夸大或虚构的内容——包装只改变视角，不编造事实
- [ ] 如果简历超过一页，适当使用 `---` 分页符
...
| "项目就是学习项目，背景写'为了学习'就行" | 学习型背景会被面试官直接降级。从技术功能反推业务场景，改用需求驱动描述。 |
| "技术难点用户没说，我就不写了" | 技术难点是面试追问入口。从代码架构决策、并发处理、性能优化中提取。没有代码就追问用户。 |
| "包装就是造假" | 包装是改变描述视角。同样的项目，"我学了Redis"vs"业务需要缓存，选择Redis因为XX"——内容一样，叙事不同。 |
